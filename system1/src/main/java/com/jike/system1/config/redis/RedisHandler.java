package com.jike.system1.config.redis;
...
import com.jike.system1.util.StringUtil;
...
public final class RedisHandler {
    ...
    public final static long TEN_MINUTES = 10 * ONE_MINUTE;
    ...
    public final static long DEFAULT_TIME_TO_LIVE = ONE_WEEK;
    ...
    public boolean expire(String key, Object value, long time) {
        try {
            if (time > 0) {
                ...
                log.warn("错误的指定时间：{}，更改为默认redis 存储时间", time);
     ...
     * @return
     */
    public boolean expire(String key, Object value) {
        try {
            this.set(key, value);
            redisTemplate.expire(key, DEFAULT_TIME_TO_LIVE, TimeUnit.MINUTES);
            return true;
        } catch (Exception e) {
            e.printStackTrace();
            return false;
    ...
    public <T> T get(String key, Class<T> clazz) {
        ...
        if (!StringUtil.checkNotEmpty(value)) {
            return null;
        }
